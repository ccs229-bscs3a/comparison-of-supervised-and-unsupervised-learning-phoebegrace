...
def app():
    ...
    text = """This is a classic example of unsupervised learning task. The Wine dataset consists 
    of chemical constituents found in wines grown in the same region in Italy but derived from 
    three different cultivars. Unlike supervised learning, it doesn't rely on labels indicating 
    the type of wine. K-means doesn't use these cultivar types during clustering.
    ...
    In essence, K-means can uncover inherent groupings within the Wine data based on the chemical 
    makeup without relying on the predefined cultivar types. 
    This exploration allows us to see how well the data naturally clusters, which may align with the 
    actual types of wine based on their chemical profiles.
    ...
    * K-means is sensitive to initial centroid placement. Running the algorithm multiple times 
    ...
    if st.button("Begin"):
        ...
        * The Wine dataset is relatively well-separated into three flower species. A Silhouette Score above 0.5 might be achievable with an appropriate number of clusters (k=3) and good initialization.
